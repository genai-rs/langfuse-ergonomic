...
async fn test_207_mixed_response() {
    ...
    let fail1 = response.failures.iter().find(|f| f.event_id == "fail-1").unwrap();
    assert!(fail1.retryable);
    assert_eq!(fail1.code, Some("500".to_string()));
    
    let fail2 = response.failures.iter().find(|f| f.event_id == "fail-2").unwrap();
...
async fn test_concurrent_flush_protection() {
    ...
    let (result1, result2) = tokio::join!(
        batcher_clone1.flush(),
        batcher_clone2.flush()
    );
